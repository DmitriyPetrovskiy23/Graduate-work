...
resource "yandex_vpc_network" "graduate" {
  name = "graduate-${var.flow}"
}

#----------create subnet zone A
resource "yandex_vpc_subnet" "graduate_a" {
  name           = "graduate-${var.flow}-ru-central1-a"
  zone           = "ru-central1-a"
  network_id     = yandex_vpc_network.graduate.id
  v4_cidr_blocks = ["10.0.1.0/28"]
  route_table_id = yandex_vpc_route_table.rt.id
}

#----------create subnet zone B
resource "yandex_vpc_subnet" "graduate_b" {
  name           = "graduate-${var.flow}-ru-central1-b"
  zone           = "ru-central1-b"
  network_id     = yandex_vpc_network.graduate.id
...
resource "yandex_alb_target_group" "target_group" {
  name           = "target-group"
  target {
    subnet_id    = yandex_vpc_subnet.graduate_a.id
    ip_address   = yandex_compute_instance.vm-web1.network_interface.0.ip_address
  }

  target {
    subnet_id    = yandex_vpc_subnet.graduate_b.id
...
#----------create alb lb
resource "yandex_alb_load_balancer" "graduate_lb" {
  name        = "graduate-lb"
  network_id  = yandex_vpc_network.graduate.id
